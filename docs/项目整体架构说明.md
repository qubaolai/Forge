# Forge 项目整体架构说明（基于当前代码实现）

> 文档范围：以仓库当前实现为准（`/Users/almond/Documents/git_data/Forge`），强调“真实代码路径与运行链路”，并标注与目标设计的差异。
...
- **Run 编排（plan_exec/workflow）**：`/runs`、`/runs/{id}/events`、`/runs/{id}/abort`、`/decisions/{token}`
