"""测试 forge.config._env 的环境变量替换逻辑."""
...
from forge.config._env import expand_env
...
def test_nested_dict(monkeypatch):
    ...
    out = expand_env({"x": "${FOO}", "y": ["${FOO}", 42], "z": {"k": "${FOO}"}})
